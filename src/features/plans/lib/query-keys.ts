...
//     → derruba lista + detail + tree + adjustments de todos os planos
//
...
//
//   queryClient.invalidateQueries({ queryKey: planKeys.adjustments(date) })
//     → derruba adjustments daquele dia (use após set/clear no B2)
//
...
export const planKeys = {
  ...
  adjustments: (dateISO: string) => ['plans', 'adjustments', dateISO] as const,
}
